...
export default function PainelProfessorPorId() {
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      const professorCarregado = await buscarJson<Professor>(
        `http://localhost:8080/professores/${professorId}`
      );

      setProfessor(professorCarregado);

      const listaAlunos = await buscarJson<Aluno[]>(
        `http://localhost:8080/alunos/professor/${professorId}`
      );
  ...
  if (!professor) {
    return (
      <div className="min-h-screen bg-[#0f172a] flex items-center justify-center text-slate-200">
        <p className="font-bold text-slate-400">Professor não encontrado.</p>
      </div>
    );
  }
          ...
          <div className="lg:col-span-1 bg-[#1e293b] p-8 rounded-3xl border border-slate-800 shadow-2xl">
            ...
            <form onSubmit={enviarMoedas} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={enviando}
                className={`w-full text-white font-black py-4 rounded-xl transition-all flex items-center justify-center gap-3 ${enviando
                    ? "bg-blue-900 cursor-not-allowed opacity-80"
